// tests/unit/RequestManager.test.js
const RequestManager = require('../../server/services/RequestManager');
const { PRIORITY, PROVIDER } = require('../../server/utils/AppConstants');
...
describe('RequestManager Unit Tests', () => {
  ...
  test('The Silent Killer: Task that never resolves', async () => {
    ...
    const validTask = RequestManager.enqueue(PRIORITY.CRITICAL, PROVIDER.MASSIVE, () => Promise.resolve('Success'));

    // Da der erste Task hängt, wird der zweite im aktuellen Loop nie erreicht, 
    // es sei denn der Manager hat einen Timeout-Schutz oder wird resettet.
